...
<table>
    ...
    <tbody>
        ...
        <tr class="odd">
            ...
            <td class="sess0 not-empty">
                <p class="cat4 presentation"><a href="#trey">On the Mineshaft Gap: Coordinating the End of Never</a></p>
...
Somewhere on your team is the person every finding eventually lands on. They spend their week reconciling scanners that disagree with each other, chasing owners for assets nobody claims, and having the same priority argument with the same overloaded engineer they had last month. When AI arrived promising to fix all of this, that person quietly picked up a new job on top of the old one: deciding whether to believe it. This session is about that decision. Ron Dovich, Chief AI and Automation Officer at Brinqa, has spent the past two years building AI into exposure management, and the plain lesson from that work is that most of it happens in the data underneath the model: getting a dozen tools to agree on what an asset even is, resolving five reports of the same flaw into one, and keeping track of which findings can reach a system the business cannot afford to lose and which person can actually fix them. Ron will walk through that work honestly, including the parts that were harder than expected, and show what changes for the analyst when an AI can point to the exact data behind every recommendation instead of asking to be taken at its word. The patterns here travel. They apply whether your team uses a vendor's AI, connects its own models, or builds from scratch, and attendees will leave with a short, practical checklist for judging any AI that makes claims about their vulnerability backlog.

*Ron Dovich is Chief AI and Automation Officer at Brinqa, where he leads the work of building AI and automation into the company's exposure management platform. He has spent more than 25 years building cybersecurity products for enterprise, government, and mid-market customers, including as AVP of Technology at AT&T, where he led product development and operations for the AT&T Business cybersecurity portfolio, and as Senior Vice President of Product Development at AlienVault, focused on threat detection and response. Earlier in his career he co-founded Blacklight Solutions and Noesis. He holds a Bachelor of Science, cum laude, from the University of Maryland.*
...
This is an active workshopping session to explore how various types of domain data can be leveraged to drive value and security decision-making for government (and other) organisations. Bring your ideas and experience!

<a name="nick-bio"></a>
*Nick Woodcraft is a technical architect in the Government Cyber Unit and the Service Owner for the Vulnerability Monitoring Service (VMS), where he leads a multi-disciplinary team providing continuous vulnerability monitoring for over 6,000 public sector organisations.*
...
<table>
    <tr>
        <td style="width:200px">
            <img style="border:1px solid black;" src="../assets/Barnabas.png" width="150" alt="Barnabás Tarcali" /><br>
            <a href="#barnabas-bio">Barnabás Tarcali, University of Groningen</a>
        </td>
        <td style="width:200px">
...
<hr>
#### <a name="trey"></a> On the Mineshaft Gap: Coordinating the End of Never

<img style="border:1px solid black;" src="../assets/trey.jpg" width="150" alt="Trey Darley" />

Trey Darley, Proper Tools SRL

Windows' native time representation doesn't have a 2038 limit: it is 64-bit. But Windows Kerberos has a 2037 problem, and it gave itself that problem to be kind to everyone else. For Linux interoperability, Windows Kerberos has long represented "never" as 13 September 2037, 02:48:05 UTC, a value squeezed into the signed 32-bit Unix range. Public packet captures show it on the wire since at least 2004.

Leverett and van der Ham-de Vos explain why an effectively unbounded population of vulnerabilities can still yield a tractable problem: their Exploitation Exposure, E = abundance × deployment-share × P(exploit), captures how exploitation filters that population down to what matters. This talk is about something E doesn't measure: whether the reachable population can actually be migrated.

The Kerberos protocol itself is fine. KerberosTime is ASN.1 GeneralizedTime, not a 32-bit epoch counter. The exposure lies where implementations convert that timestamp into narrower native representations. A public 2022 test report describes domain logon failing as the old "never" date approaches, and a Microsoft engineer has confirmed that the boundary is real in pre-2025 Windows environments.

Microsoft has moved "never" to 2100: Windows 11 22H2-era clients moved first, followed by Windows Server 2025 KDCs. 2100 fits unsigned 32-bit time but overflows a signed 32-bit time_t. The consequences were predicted in public in 2022, when a Heimdal maintainer warned that "a year of 2100 will still cause a failure." They arrived as independent breakages anyway: Cisco ISE and 32-bit MIT krb5 clients in 2025, followed by an upstream MIT fix in March 2026 that clamps the value. As of September 2026, that fix still has not shipped in a stable MIT release. Meanwhile, Microsoft runs documented, staged transitions for NTLM, NTLMv1 and RC4 in the same authentication ecosystem. For this one: forum replies, yes; visible machinery, no.

Every party's decision was locally reasonable, and that is the problem. The dangerous population during a transition may be neither the remediated systems nor the unremediated ones, but the seam between them, which nobody owns. The talk closes with a practical, testable thought experiment: rather than rolling clocks forward, use Server 2025's post-2038 Kerberos timestamps as a nondestructive probe for 32-bit time assumptions elsewhere in an estate.

This talk is also a correction. For a year I described Kerberos as having a protocol-level 2038 problem. Nobody corrected or challenged me. I found out I was wrong days before presenting, by piecing the real story together from a detection engineer's blog, a mailing list, a vendor forum, a bug tracker, and a misfiled support thread. If it takes that much to establish what is true about one protocol, the knowledge this transition depends on will not assemble itself.
